...
- **`BaseLayout` props:** `title`, `description`, `lang`, optional `alternatePath` (drives hreflang alternates and x-default), and `noindex` — set by the 404 pages, it suppresses canonical/alternates/og:url/JSON-LD and emits `robots: noindex, nofollow`. Canonical URLs are built on `https://insposoft.com`.
